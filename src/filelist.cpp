...
void FileList::dropEvent( QDropEvent *event )
{
    ...
    for( int i=0; i<q_urls.size(); i++ )
    {
        ...
        else
        {
            ...
            fileName = QUrl(q_urls.at(i)).url(QUrl::PreferLocalFile);
...
void FileList::updateItem( FileListItem *item )
{
    ...
    {
        item->setText( Column_Input, item->url.url(QUrl::PreferLocalFile) );
    }
}
...
void FileList::load( bool user )
{
    ...
    {
        const int ret = QMessageBox::question( this, i18n("Overwrite current file list"), i18n("Do you want to overwrite the current file list?\n\nIf not, the saved file list will be appended.") );
    ...
    QFile listFile( QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) + "/soundkonverter/" + fileName );
...
void FileList::save( bool user )
{
    ...
    for( int i=0; i<topLevelItemCount(); i++ )
    {
        ...
        file.setAttribute("url",item->url.url(QUrl::PreferLocalFile));
        // file.setAttribute("outputUrl",item->outputUrl.url(QUrl::PreferLocalFile));
